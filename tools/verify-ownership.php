...
foreach($source as $entry){if(($entry['origin']??null)==='package'){if(!is_file($root.'/'.$entry['target_path'])||$entry['source_repository']!=='https://github.com/'.json_decode(file_get_contents($root.'/composer.json'),true)['name'])throw new RuntimeException('Invalid package-owned source entry.');continue;}if(!is_file($root.'/'.$entry['target_path'])||preg_match('/^[a-f0-9]{64}$/D',$entry['source_sha256'])!==1||preg_match('/^[a-f0-9]{40}$/D',$entry['source_commit'])!==1)throw new RuntimeException('Missing exact source evidence.');}
