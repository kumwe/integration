...
Public types retain their canonical ownership and compatibility provenance. ConfigProvider and Container/EventContractRegistryFactory add package composition without an App dependency. The host supplies the canonical encoder and exact declaration objects, selects the trusted generation and controls authority. App retains durable transaction-plus-outbox, inbox contention, replay, egress/security and worker lifecycle tests.
